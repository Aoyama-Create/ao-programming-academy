...
VS Code には「ソース管理」の機能があります。左のアイコンから「ソース管理」を開くと、変更したファイルの一覧が出て、チェックマーク（✓）でステージング、上部の「コミット」で commit ができます。ターミナルで `git push` するか、画面上の「同期」などで push できます。慣れてきたら、ターミナルと併用するとよいです。
